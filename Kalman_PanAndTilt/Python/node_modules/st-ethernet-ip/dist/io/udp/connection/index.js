...
class Connection extends EventEmitter {
    constructor(port = 2222, address, config, rpi = 10) {
        super();
        this.tcpController = new tcp_1.default(true, config.configInstance, config.outputInstance, config.inputInstance);
    ...
    _connectTCP() {
        ...
        this.TOsequenceNum = 0;
        this.cipCount = 0;
        this.tcpController = new tcp_1.default(true, this.config.configInstance, this.config.outputInstance, this.config.inputInstance);
    ...
    _checkStatus() {
        if (Date.now() - this.lastDataTime > this.tcpController.rpi * 4) {
            ...
            if (!this.tcpController.state.TCP.establishing && this.connected && this.run)
                setTimeout(() => this._connectTCP(), this.rpi * 20);
            this.connected = false;
        }
        else {
